resource "null_resource" "provisioner" {
  depends_on = [data.template_file.inventory]
  for_each   = { for server in local.hosts : server.hostname => server }

  triggers = {
    hostname = each.value.hostname
    ip       = each.value.ip
    playbook = "${sha1(file("playbooks/rke_nodes.yml"))}"
    role     = each.value.role[0]
  }
  connection {
    type        = "ssh"
    host        = self.triggers.ip
    user        = "root"
    private_key = file(local.ssh_key_path)
  }
  provisioner "remote-exec" {
    inline = [
      "echo ${join(" ", each.value.role)} > /tmp/roles"
    ]
  }
  provisioner "local-exec" {
    command = "ansible-playbook  -i ${local_file.inventory[each.value.hostname].filename} --private-key=${local.ssh_key_path} playbooks/rke_nodes.yml -t provision"
  }
}
